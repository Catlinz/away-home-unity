...
public class ModuleSystem {
    ...
    public delegate void ModuleInstalled(ActorModuleClass module);
    ...
    public delegate void ModuleRemoved(ActorModuleClass module, RemovedReason reason);
    ...
    public delegate void ModuleEnabled(ActorModuleClass module);
    ...
    public delegate void ModuleDisabled(ActorModuleClass module);
    ...
    /// </summary>
	public ModuleSystem() {
        groups = new HardPointGroup[(int)HardPointGroup.Index.Passive + 1];
        groups[(int)HardPointGroup.Index.Primary] = new HardPointGroup("Primary", HardPointGroup.Flags.Targeted);
		groups[(int)HardPointGroup.Index.Secondary] = new HardPointGroup("Secondary", HardPointGroup.Flags.Targeted);
		groups[(int)HardPointGroup.Index.Utility] = new HardPointGroup("Utility", HardPointGroup.Flags.Utility);
		groups[(int)HardPointGroup.Index.Passive] = new HardPointGroup("Passive", HardPointGroup.Flags.Passive);
	}
	...
	public bool CanEnable(ActorModuleClass module, ShipActorComponent ship) {
		return (ship.computer.IdleResources >= module.idleComputerResources) && (ship.power.FreeEnergy >= module.idleEnergyDrain);
	}
	...
	/// </returns>
	public OperationResult CanInstallInSocket(InstallableModuleAsset moduleAsset, HardPoint socket) {
	...
	public OperationResult AddSocketToGroup(string socketName, string groupName) {
		HardPoint toAdd = GetSocket(socketName);
		HardPointGroup addTo = GetSocketGroup(groupName);
		...
		HardPointGroup prevGroup = GetSocketGroupFor(socketName);
		if (prevGroup != null) {
    ...
    public HardPointGroup CreateSocketGroup(string newName) {
        if (HasGroup(newName)) { return null; }

        HardPointGroup newGroup = new HardPointGroup(newName);
    ...
    public HardPointGroup GetSocketGroup(string groupName) {
	...
	public HardPointGroup GetSocketGroupFor(string socketName) {
		for (int i = 0; i < groups.Length; ++i) {
	...
	public HardPoint GetSocket(string socketName) {
    ...
    public HardPoint GetSocket(ActorModuleClass module) {
        for (int i = 0; i < sockets.Length; ++i) {
    ...
    public InstallableModuleAsset RemoveModuleFrom(HardPoint socket, ShipActorComponent ship, RemovedReason reason = RemovedReason.Uninstalled) {
        TryDisable(socket, ship);

        ActorModuleClass module = socket.Module;
    ...
    public void ResetSocket(HardPoint socket) {
    ...
    /// </returns>
    public OperationResult TryDisable(HardPoint socket, ShipActorComponent ship) {
    ...
    /// </returns>
	public OperationResult TryEnable(HardPoint socket, ShipActorComponent ship) {
        if (socket.Module == null) {
