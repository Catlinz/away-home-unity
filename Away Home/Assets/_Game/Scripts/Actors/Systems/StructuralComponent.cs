...
public class StructuralComponent : MonoBehaviour, ISystem {
	...
	public bool CanEnableModule(ActorModule module) {
		return 	(_system.computer.IdleResources >= module.idleComputerResources) &&
				(_system.power.FreeEnergy >= module.idleEnergyDrain);
	}
	...
	public OperationResult EnableModule(Hardpoint hardpoint) {
		...
		if (CanEnableModule(module)) {
			...
			if (module.idleEnergyDrain > _system.power.FreeEnergy) {
